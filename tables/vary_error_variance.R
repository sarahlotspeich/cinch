...
res = read.csv("~/Documents/cinch/simulations/vary_error_variance.csv")
...
res_summ = res |> 
  dplyr::group_by(true_ci, error_var) |> 
  dplyr::summarize(
    ## Absolute bias 
    ...
    ## Relative (%) bias 
    perc_bias_gs_ci = mean((gs_ci - true_ci) / true_ci),
    perc_bias_mb_ci = mean((mb_ci - true_ci) / true_ci),
    perc_bias_nv_ci = mean((nv_ci - true_ci) / true_ci),
...
res_summ |> 
  dplyr::arrange(true_ci, error_var) |> 
  dplyr::select(true_ci, error_var, 
