...
for (ci in c(-0.5, 0, 0.5)) {
  for (times_more_error in c(1.1, 1.5, 2)) {
    ...
    for (r in 1:num_rep) {
      ...
      ## Simulate data for group 2 (more error-prone)
      dat2 = sim_data(error_sd = sqrt(sigma2U * times_more_error), 
                      n = N / 2, 
                      approx_ci = ci, 
                      pv = pv, 
                      design = "SRS")
      
      ## Stack the two groups' data 
      dat = rbind(dat1, dat2)
      ...
      df_res = data.frame(
        true_ci = ci, 
        less_error_var = sigma2U,
        more_error_var = sigma2U * times_more_error,
        gs_ci = oracle_ci, 
      ...
      df_res |> 
        write.csv(file = paste0("simulations/vary_error_depend.csv"), 
                row.names = FALSE)  
    }
  }
}