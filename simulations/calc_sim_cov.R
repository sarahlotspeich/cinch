...
## Plot Var(R) and Var(R*) as N increases
covRW = varW = varR = varRstar = vector()
for (main_study in c(25, 50, 100, 250, 500, 1000, 2500, 5000)) {
  temp = sim_data(error_sd = sqrt(0.5), 
                  n = main_study, 
                  approx_ci = -0.5, 
                  pv = 0.1, 
                  design = "SRS")
  varR = append(varR, with(temp, var(R)))
  varRstar = append(varRstar, with(temp, var(Rstar)))
  covRW = append(covRW, with(temp, cov(R, W)))
  varW = append(varW, with(temp, var(W)))
}
data.frame(N = c(25, 50, 100, 250, 500, 1000, 2500, 5000), 
           varR, varRstar, covRW, varW) |> 
  ggplot(aes(x = N)) + 
  geom_line(aes(y = varR), color = "blue") + 
  geom_line(aes(y = varRstar), color = "red") + 
  geom_line(aes(y = -covRW), color = "green") + 
  geom_line(aes(y = varW), color = "purple")
